...
def load_model(folder, x_train, y_train, x_val, y_val):
    with open(f"{folder}/initial_model.pkl", "rb") as file:
        base_model = pkl.load(file)
    pdl_reg = PointerDecisionList(base_model, x_train, y_train, x_val, y_val, alpha = 1, min_group_size = 1)
    pdl_reg.reload_model(folder)
    return pdl_reg
class node:
    '''
    Substructure of the PDL class. Stores instructions for predicting on an instance within a level set at a given point in time for the PDL structure.

    Inputs:
        - self: self
        - right_child: next node in PDL sequence (node)
...
class PointerDecisionList:
    '''
    The PDL structure is a list of nodes which each contain conditional statements for predicting on instances within a level set.
    ...
    def compute_group_errors(self, y_train, y_val):
        ...
        for index in range(len(self.group_indices_train)):
            ...
            group_mse_train = loss_fn(y_train[group_indices],self.train_predictions[group_indices])
        ...
        for index in range(len(self.group_indices_val)):
            ...
            group_mse_val = loss_fn(y_val[group_indices],self.val_predictions[group_indices])
